...
tokeniza (?) um texto usando tokens selecionados com o metodo greedy longest-\/matchconverte cada frase do texto em um vetor de IDs de tokens seleciona o token mais longo possível em cada posição do texto tokens não encontrados no vocabulário são representados pelo valor -\/1 (Símbolo Desconhecido ou SD) Complexidade\+: O(n \texorpdfstring{$\ast$}{*} m \texorpdfstring{$\ast$}{*} k) onde n é número de frases, m é comprimento das frases, k é número de tokens
