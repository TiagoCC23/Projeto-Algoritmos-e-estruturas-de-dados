...
void substituirParNoTexto(MATRIX_STR *text, char *par, char novoIdToken) {
    ...
    for (int i = 0; i < text->count; i++) {
        ...
        char *newSubStr = malloc(tam * 2 + 1);

        int p = 0, j = 0;

        while (j < tam) {
            if (j < tam - 1 && subStr[j] == par[0] && subStr[j + 1] == par[1]) {
                newSubStr[p++] = novoIdToken;
                j += 2;

            } else {
                newSubStr[p++] = subStr[j++];
            }
        }
        newSubStr[p] = '\0';
        free(text->strings[i]);
...
void merge(MATRIX_STR *tokens, MATRIX_STR *matr) {
    ...
    for (int i = 0; i < matr->count; i++) {                                               // criar os pares consecutivos
        ...
        for (int j = 0; j < tam - 1; j++) {
            ...
            auxSubStr[0] = string[j];
            auxSubStr[1] = string[j + 1];
            auxSubStr[2] = '\0';
    ...
    if (idToken1 < tokens->count) {                                          // se nao ele divide a letra em duas partes
    ...
    char *tokenFinal = malloc(strlen(parte1) + strlen(parte2) + 1);
    ...
    for (int i = 0; i < pos; i++) free(freqString[i]);
    free(freqString);
